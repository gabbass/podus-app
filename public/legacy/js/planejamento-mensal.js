...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const respMaps = await fetch('/api/planejamentos?acao=bncc&campo=mapas');
    const mapsJson = await respMaps.json();
    if (!mapsJson.sucesso) {
      throw new Error(mapsJson.mensagem || 'Falha ao carregar mapas da BNCC');
    }

    const mapsData = mapsJson.dados || {};
    window.bnccOptions = mapsData.options || {};
    window.bnccLookups = mapsData.lookups || {};
    window.bnccMaps = window.bnccLookups; // compatibilidade com código legado

    // 2) Carrega template de linha (uma única vez)
    const respTpl = await fetch('/planejamentos/linha');
...
async function inicializarPlanejamentoMensal() {
  ...
  if (selectTempo) {
    try {
      const resp = await fetch('/api/planejamentos?acao=listar_ciclos');
  ...
  const selectMateria = document.getElementById('materia');
	if (selectMateria) {
	  fetch('/api/planejamentos?acao=materias_do_professor')
		.then(r => r.json())
		.then(lista => {
		  selectMateria.innerHTML = '<option value="" disabled selected>Selecione a matéria</option>';
                  ...
                  lista.forEach(m => {
                        ...
                        materiasLookup[String(m.id)] = m.label;
...
async function salvarCadastroPlanejamentoMensal() {
  ...
  try {
    const r = await fetch('/api/planejamentos',
                          { method: 'POST', body: fd });
...
async function editarPlanejamentoMensal(id) {
  try {
    ...
    const resp = await fetch(`/api/planejamentos?acao=buscar&id=${id}`);
...
async function listarPlanejamentos(termoPesquisa = '') {
  ...
  try {
    const resp = await fetch(`/api/planejamentos?acao=buscar_todos&pesquisa=${encodeURIComponent(termoPesquisa)}`);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const json = await resp.json();
    if (!json.sucesso) throw new Error(json.mensagem || 'Erro ao listar planejamentos');

    tbody.innerHTML = '';
    json.data.forEach(plano => {
      const materiaNome = materiasLookup[String(plano.materia)]
        || materiasLookup[String(plano.materia_id)]
        || plano.materia
        || '';
      ...
      tr.innerHTML = `
        ...
        <td class="d-flex gap-2">
            <button class="btn-action btn-view" onclick="window.location.href='/planejamentos/${plano.id}'"><i class="fas fa-eye"></i></button>
    ...
    if (contador) {
      contador.innerHTML = `Exibindo <strong>${json.data.length}</strong> registro(s) encontrado(s)`;
    }
  } catch (e) {
...
async function injetarBlocoVisual(gid, dados) {
  ...
  const resp = await fetch(`/planejamentos/linha?gid=${gid}`);
...
// ---------------------------------------------------------------------------
		...
		if (ch) {
			...
			ch.clearChoices();
                        ...
                        // 3.2 Constrói array de opções a partir do catálogo BNCC carregado na página
                        const listaHab = (window.bnccOptions?.habilidades || []).map(item => ({
                                value : String(item.id),   // Choices requer value em string
                                label : item.label          // Texto que aparece ao usuário
                        }));
...
/**
 * Popula um <select> de BNCC com base nas opções carregadas da API
...
function carregarOpcoesNoSelect(idSelect, entidade, valorSelecionado = null) {
  ...
  if (!sel) return;
  const options = window.bnccOptions || {};
  const itens = options[entidade] || [];
