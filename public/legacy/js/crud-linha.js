...
function reiniciarCascataBNCC(gid) {
  ...
  // Carrega Etapas
  fetch('/api/planejamentos?acao=bncc&campo=etapas')
    .then(r => r.json())
    .then(json => {
...
function getNomeBNCC(tipo, id) {
  const lookups = window.bnccLookups || {};
  const chave = id != null ? String(id) : '';

  if (!chave) return '';

  if (tipo === 'habilidades') {
    const habilidades = lookups.habilidades || {};
    const habilidadesLinha = lookups['habilidades-linha'] || {};
    return habilidadesLinha[chave] || habilidades[chave] || chave;
  }

  return lookups[tipo]?.[chave] || chave;
}
...
function editarLinha(idx) {
  ...
  gid
    ...
    .then(() => {
  return fetch(`/api/planejamentos?acao=bncc&campo=habilidades&id_objeto=${l.objetosConhecimento}`)
    .then(r => r.json())
    .then(json => {
      const lista = Array.isArray(json.dados) ? json.dados : [];
...
function carregarSelectESelecionar(baseId, entidade, filtros = {}, valorSel = null, grupoMostrar = null, gid) {
  ...
  return fetch(`/api/planejamentos?acao=bncc&campo=${entidade}&${new URLSearchParams(filtros)}`)
    .then(r => r.json())
    .then(json => {
      const lista = Array.isArray(json.dados) ? json.dados : [];
      lista.forEach(it => {
      ...
      if (valorSel !== null) {
        const valorStr = String(valorSel);
        sel.value = valorStr;
        if (sel.value !== valorStr) {
          const optMatch = Array.from(sel.options).find(opt => opt.value === valorStr);
          if (optMatch) {
            optMatch.selected = true;
          } else {
            const optByText = Array.from(sel.options)
              .find(opt => opt.textContent.trim().toLowerCase() === valorStr.toLowerCase());
            if (optByText) optByText.selected = true;
          }
        }
      }
      if (grupoMostrar) {
...
function bindSequencialBNCC(gid) {
  ...
  if (objSel) objSel.onchange = function() {
    ...
    fetch(`/api/planejamentos?acao=bncc&campo=habilidades&id_objeto=${this.value}`)
      .then(r => r.json())
      .then(json => {
        const lista = Array.isArray(json.dados) ? json.dados : [];
        const ch = window.choicesHabilidadesMap[gid];
