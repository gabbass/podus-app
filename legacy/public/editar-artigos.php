...
try {
    // Buscar artigo
    $stmt_busca = $conexao->prepare("SELECT * FROM artigo WHERE id = ?");
        ...
        if (empty($nome) || empty($texto)) {
            ...
            if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($imagem_tipo, $tipos_permitidos)) {
                    $mensagem .= "<div class='alert alert-warning'>Tipo de imagem não permitido. Use JPG, JPEG, PNG ou GIF.</div>";
                } else {
                    if (move_uploaded_file($_FILES['imagem']['tmp_name'], $caminho_destino)) {
                        ...
                        $caminho_imagem = $caminho_destino;
                    } else {
                        $mensagem .= "<div class='alert alert-danger'>Erro ao fazer upload da nova imagem.</div>";
                    }
                }
            }

            // Atualizar no banco
            $stmt_update = $conexao->prepare("UPDATE artigo SET nome = ?, texto = ?, fonte = ?, imagem = ?, data = ? WHERE id = ?");
            $stmt_update->execute([$nome, $texto, $fonte, $caminho_imagem,$data, $id_artigo]);

            $mensagem = "<div class='alert alert-success'>Artigo atualizado com sucesso!</div>";
            // Atualiza os dados para exibir os novos valores
            $artigo = [
                'nome' => $nome,
                'texto' => $texto,
                'fonte' => $fonte,
    ...
    $mensagem = "<div class='alert alert-danger'>Erro ao salvar alterações: " . $e->getMessage() . "</div>";
}
?>
...
<head>
    ...
    <title>Editar Artigo - Universo do Saber</title>
        ...
        <div class="content">
            <div class="page-header">
                <div class="page-title">
                    <h1>Editar Artigo</h1>
                    <p>Atualize os campos abaixo para modificar o artigo</p>
                </div>
                <a href="pesquisar-artigos.php" class="btn btn-secondary">
            ...
            <div class="card">
                <?php if (!empty($artigo)): ?>
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="data" id="data" value="<?= date('Y-m-d',($artigo['data'])) ?>" required>
                        </div>

                        
                        <div class="form-group">
                            <label for="nome">Título do Artigo:</label>
                            <input type="text" name="nome" id="nome" value="<?= htmlspecialchars($artigo['nome']) ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="texto">Texto do Artigo:</label>
                            <textarea name="texto" id="texto" required><?= htmlspecialchars($artigo['texto']) ?></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label for="fonte">Fonte (opcional):</label>
                            <input type="text" name="fonte" id="fonte" value="<?= htmlspecialchars($artigo['fonte']) ?>">
                        </div>
                        
                        <div class="form-group">
                            <label>Imagem Atual:</label><br>
                            <?php if (!empty($artigo['imagem']) && file_exists($artigo['imagem'])): ?>
                                <img src="<?= $artigo['imagem'] ?>" alt="Imagem do artigo" class="img-preview">
                            <?php else: ?>
                                <small>Nenhuma imagem definida</small>
                            <?php endif; ?>
                        </div>
                ...
                <?php else: ?>
                    <div class="alert alert-danger">Artigo não encontrado ou não foi possível carregar.</div>
                <?php endif; ?>
            </div>
        </div>
    </div>
