...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validações básicas
    if (empty($nome) || empty($email) || empty($login) || empty($escola)) {
        $erro = 'Todos os campos são obrigatórios!';
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $erro = 'E-mail inválido!';
    } else {
        try {
            // Verifica se o login já existe (exceto para o próprio professor)
            $query_check = "SELECT id FROM login WHERE login = :login AND id != :id";
            $stmt_check = $conexao->prepare($query_check);
            $stmt_check->bindParam(':login', $login);
            $stmt_check->bindParam(':id', $id_professor, PDO::PARAM_INT);
            $stmt_check->execute();
            ...
            if ($stmt_check->fetch()) {
                ...
                $stmt_update->bindParam(':login', $login);
                $stmt_update->bindParam(':escola', $escola);
                $stmt_update->bindParam(':id', $id_professor, PDO::PARAM_INT);
...
<head>
    ...
    <title>Editar Professor - Universo do Saber</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="pesquisar-professores" class="active"><i class="fas fa-chalkboard-teacher"></i> <span>Professores</span></a></li>
        ...
        <div class="content">
            <div class="page-header">
                <div class="page-title">
                    ...
                    <p>Edite as informações do professor</p>
                </div>
                <a href="pesquisar-professores" class="btn btn-secondary">
            ...
            <div class="card">
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="login" name="login" value="<?= htmlspecialchars($professor['login']) ?>" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="escola">Escola</label>
                        <input type="text" id="escola" name="escola" list="lista-escolas" value="<?= htmlspecialchars($professor['escola']) ?>" required>
                        <datalist id="lista-escolas">
                            <?php include 'escolas.php'; ?>
                        </datalist>
                    </div>
