...
$campos = [
    ...
    'tipo' => $_COOKIE['planejamento_tipo'] ?? 'Bimestral',
    'sequencial' => $_COOKIE['planejamento_sequencial'] ?? '1º',
    'diagnostico' => $_COOKIE['planejamento_diagnostico'] ?? '', // Corrigido para usar cookie
    'referencias' => $_COOKIE['planejamento_referencias'] ?? '', // Corrigido para usar cookie
    'unidade_tematica' => '',
    'objeto_do_conhecimento' => $_COOKIE['planejamento_objeto_do_conhecimento'] ?? '',
    'conteudos' => '',
    'habilidades' => '',
    'metodologias' => '',
    'projetos_integrador' => $_COOKIE['planejamento_projetos_integrador'] ?? ''
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $campos = [
        ...
        'data' => trim(strtotime($_POST['data']) ?? ''),
        ...
        'escola' => $_SESSION['escola'] ?? trim($_POST['escola'] ?? ''),
        'professor' => $_SESSION['nome'] ?? trim($_POST['professor'] ?? ''),
        'login' => $_SESSION['login'] ?? trim($_POST['login'] ?? ''),
        'ano' => trim($_POST['ano'] ?? ''),
    ...
    if (empty($erros)) {
        try {
            $sql = "INSERT INTO planejador (data,
                curso, componente_curricular, objetivo_geral, objetivo_especifico, escola, 
                professor, login, ano, numero_aulas_semanais, tipo,sequencial,
                unidade_tematica, objeto_do_conhecimento, conteudos, habilidades, 
                metodologias, projetos_integrador, diagnostico, referencias
            ) VALUES (:data,
                :curso, :componente_curricular, :objetivo_geral, :objetivo_especifico, :escola,
                :professor, :login, :ano, :numero_aulas_semanais, :tipo,:sequencial, :unidade_tematica, :objeto_do_conhecimento, :conteudos, :habilidades,
                :metodologias, :projetos_integrador, :diagnostico, :referencias
            )";
            ...
            setcookie('planejamento_data', date('Y-m-d'), $cookie_expiry, '/');
            setcookie('planejamento_curso', $campos['curso'], $cookie_expiry, '/');
            setcookie('planejamento_sequencial', $campos['sequencial'], $cookie_expiry, '/');
            ...
            setcookie('planejamento_tipo', $campos['tipo'], $cookie_expiry, '/');
            setcookie('planejamento_diagnostico', $campos['diagnostico'], $cookie_expiry, '/');
            setcookie('planejamento_referencias', $campos['referencias'], $cookie_expiry, '/');
            setcookie('planejamento_objeto_do_conhecimento', $campos['objeto_do_conhecimento'], $cookie_expiry, '/');
            setcookie('planejamento_projetos_integrador', $campos['projetos_integrador'], $cookie_expiry, '/');
            
            
            echo "<script>alert('Planejador Cadastrado com sucesso!')</script>";
            echo "<script>location.assign('planejador-cadastrar')</script>";
            ...
            $campos = [
                ...
                'tipo' => $_COOKIE['planejamento_tipo'] ?? 'Bimestral',
                'sequencial' => $_COOKIE['planejamento_sequencial'] ?? '1º',
                'diagnostico' => $_COOKIE['planejamento_diagnostico'] ?? '',
                'referencias' => $_COOKIE['planejamento_referencias'] ?? '',
                'unidade_tematica' => '',
                'objeto_do_conhecimento' => $_COOKIE['planejamento_objeto_do_conhecimento'] ?? '',
                'conteudos' => '',
                'habilidades' => '',
            ...
            <div class="card">
                <form method="POST" action="">
                    <div class="form-group">
                        ...
                        <input type="date" id="data" name="data" value="<?= $_COOKIE['planejamento_data'] ?? date('Y-m-d') ?>">
                    </div>
                    ...
                    <div class="form-group">
                        <label for="unidades_tematica">Unidades Temáticas</label>
                        <select name='unidade_tematica' id='unidades_tematica'>
                        <?php
...
<div class="form-group">
    ...
    <select name="habilidades" id="habilidades">
        ...
        try {
            $sql = "SELECT habilidade FROM habilidades"; // Consulta ao banco de dados
            $stmt = $conexao->prepare($sql);
            $stmt->execute();
    ...
    function limparCookies() {
        if (confirm('Tem certeza que deseja limpar todos os dados salvos automaticamente?')) {
            ...
            document.cookie = "planejamento_data=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            ...
            document.cookie = "planejamento_sequencial=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            ...
            document.cookie = "planejamento_tipo=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            document.cookie = "planejamento_diagnostico=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            document.cookie = "planejamento_referencias=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            document.cookie = "planejamento_objeto_do_conhecimento=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            document.cookie = "planejamento_projetos_integrador=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
