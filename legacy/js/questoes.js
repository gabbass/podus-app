...
const linha = q => `<tr>
	<td>
		<input type="checkbox" class="checkbox-selecao" data-id="${q.id}">
	</td>
  <td>${q.id}</td>
...
async function listar(p=1){
  ...
  }catch(e){ mostrarAlerta(e,'danger'); }
}
...
window.delQ  = id=>{
  showModalGeral({
    ...
    onConfirm: async ()=>{
      ...
      j.sucesso?listar(ST.pag):mostrarAlerta(j.msg,'danger');
    }
  });
};
...
async function carregar(id){
  try{
	...
	if (sel) {
	  sel.value = j.dado.resposta;
	  sel.dispatchEvent(new Event('change'));
	}
	
	 if (choicesResposta) {
    choicesResposta.setChoiceByValue(j.dado.resposta);
    // opcional: dispara o change para seu highlight
    choicesResposta.passedElement.element.dispatchEvent(new Event('change'));
	}
...
function lockForm() {
  ...
  $jq?.fn.summernote && $jq('.summernote, .summernote-small').summernote('disable');
   ...
   const btnAll = dom('btnCancelarTudo');
  if (btnAll) {
    btnAll.disabled = false;
    btnAll.classList.remove('bg-light');
  }
}
...
function salvar(ev){
 ...
 fetch('includes/api-questoes.php',{ method:'POST', body:fd })
    ...
    .then(j => {
      if (!j.sucesso) throw j.msg;
      mostrarAlerta('Questão salva com sucesso','success');
...
document.addEventListener('DOMContentLoaded',()=>{
  /* LISTA */
  const resp = dom('resposta');
  if (!resp) return;

  choicesResposta = new Choices(resp, {
    choices: [
      { value: 'A', label: 'A' },
      { value: 'B', label: 'B' },
      { value: 'C', label: 'C' },
    ...
    searchEnabled: false,    // sem busca
    shouldSort: false,       // mantém ordem A→E
    itemSelectText: '',      // sem texto “Press to select”
    placeholder: true,
    placeholderValue: 'Selecione'
  });
  
  // listener nativo para highlight das divs
    resp.addEventListener('change', () => {
      const opc = resp.value.trim().toUpperCase();
      ['A','B','C','D','E'].forEach(l => {
        const w = dom(`wrapper_alternativa_${l}`);
        if (w) w.classList.remove('selecionada');
      });
      const certo = dom(`wrapper_alternativa_${opc}`);
      if (certo) certo.classList.add('selecionada');
    });
...
// Escuta todas as mudanças nos checkboxes
document.addEventListener('change', function (e) {
  // Seleção individual
  if (e.target && e.target.classList.contains('checkbox-selecao')) {
    const id = e.target.dataset.id;
    if (e.target.checked) {
      questoesSelecionadas.add(id);
    } else {
      questoesSelecionadas.delete(id);
    }
	 atualizarVisibilidadeBotoesGerarProva();
  }

  // Selecionar todos
  if (e.target && e.target.id === 'checkTodos') {
    const todos = document.querySelectorAll('.checkbox-selecao');
    todos.forEach(cb => {
...
function atualizarVisibilidadeBotoesGerarProva() {
  const btnImp   = document.getElementById('btnGerarProva');
  const btnOnline= document.getElementById('btnGerarProvaOnline');
  const qtd      = questoesSelecionadas.size;

  if (btnImp && btnOnline) {
    if (qtd >= 2) {
      btnImp.classList.remove('oculto');
      btnOnline.classList.remove('oculto');
    } else {
      btnImp.classList.add('oculto');
      btnOnline.classList.add('oculto');
    }
  }
}
